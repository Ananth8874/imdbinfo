...
for movie in results.titles:
    print(
        f"Found a movie: {movie.title} ({movie.title_localized}) - {movie.imdbId} of kind {movie.kind}"
    )
...
for imdb_id in movies_list:
    ...
    if movie.is_series():
        ...
        print(
            "##### this is a SERIES: extra info available in movie.info_series  #####"
        )
        print(
            "#########################################################################"
        )
        ...
        print(
            "##### this is an EPISODE: extra info available in movie.info_episode #####"
        )
        print(
            "##########################################################################"
        )
    ...
    print(
        f"Country Codes: {', '.join(movie.country_codes) if movie.country_codes else 'N/A'}"
    )
    print(f"Stars:")
    ...
    print("Cast:")
    for cast_member in movie.categories["cast"][
        :3
    ]:  # Limit to the first 3 cast members for brevity
